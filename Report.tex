...
В моделі RMA (Random Plaintext Attack), також відомій як KPA (Known Plaintext Attack), супротивник перехоплює набір 
...
Message Attack, sEU a.k.a. SUF), яке додатково запобігає підробці зловмисником дійсного підпису для будь-якого 
...
Для фіксованого типу атаки ATK~\cite{GoldwasserMicaliRivest1988, KatzLindell2020}:
\begin{equation*}
...
Повна ієрархія рівнів стійкості для схем підпису зображена на рисунку~\ref{fig:signature-hierarchy}.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % sEU рівень (верхній)
        \node[box] (seucma) {sEU-CPA};
        \node[box, right=of seucma] (seurma) {sEU-RMA};
        \node[box, right=of seurma] (seukoa) {sEU-KOA};

        % EU рівень (середній)
        \node[box, below=of seucma] (eucma) {EU-CPA};
        \node[box, right=of eucma] (eurma) {EU-RMA};
        \node[box, right=of eurma] (eukoa) {EU-KOA};

        % UU рівень (нижній)
        \node[box, below=of eucma] (uucma) {UU-CPA};
        \node[box, right=of uucma] (uurma) {UU-RMA};
        \node[box, right=of uurma] (uukoa) {UU-KOA};
        ...
        % Горизонтальні стрілки (CPA -> RMA -> KOA)
    ...
    \end{tikzpicture}
    \caption{Ієрархія рівнів стійкості схем цифрового підпису. Стрілки позначають імплікації (від сильнішого до слабшого).}
